...
SAL_f64 PREFIX( fftd_tbuf[2048] );

void PREFIX( _csal_cfftsmd )( c, stride, n, f, brflag )
SAL_f64 *c;
SAL_i32    stride;
SAL_i32    n;
SAL_i32    f;
...

{
    ...
    for ( i = 0; i < n; i++ )
    {
        PREFIX( fftd_tbuf[j] ) = c[k];
        PREFIX( fftd_tbuf[j+1] ) = c[k+1];
        j += 2;
        k += stride;
    }
    ...
    for ( i = 0; i < n; i++ )
    {
        c[k] = PREFIX( fftd_tbuf[j] );
        c[k+1] = PREFIX( fftd_tbuf[j+1] );
        j += 2;
        k += stride;
    }
...
void PREFIX( _csal_bitrd )( c, log2n )
SAL_f64 *c;
SAL_i32    log2n;
{
    SAL_i32   i, j, k, n, nm1;
...
void PREFIX( _csal_freald )( n, top, bot, w, winc )
SAL_i32    n;
SAL_f64 *top;
SAL_f64 *bot;
SAL_f64 *w;
...
void PREFIX( _csal_ireald )( n, top, bot, w, winc )
SAL_i32    n;
SAL_f64 *top;
SAL_f64 *bot;
SAL_f64 *w;
