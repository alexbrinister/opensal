...
)
{
    ...
    if ( FLAG > 0 )
    {
        ...
        {
            PREFIX( _csal_fft_zbd )( &Ap, IR, &Cp, KR,
        ...
        PREFIX( _csal_fft_zbd )( &Cp, ( KC << 1 ), &Cp, ( KC << 1 ),
    ...
    else
    {
        ...
        Cp = *C;
        Cp.imagp = Cp.realp + KC;
        PREFIX( _csal_fft_zrpd )( &Ap, ( IC << 1 ), T, ( SAL_i32 )1,
                                  LOG2NR, FLAG, SETUPP );
        ...
        for ( i = 1; i < ( SAL_ui32 )( 1 << ( LOG2NC - 1 ) ); i++ )
        {
            PREFIX( _csal_fft_zbd )( &Ap, IC, T, ( SAL_i32 )1, &Cp, KC, LOG2NR, FLAG,
                                     SETUPP );
            Ap.realp += IR;
            Ap.imagp += IR;
            Cp.realp += KR;
