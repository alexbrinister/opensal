...
void  PREFIX( _csal_fft_zrpd )( SAL_zf64 *A,
                                SAL_i32 I,
                                SAL_zf64 *C,
                                SAL_i32 K,
                                SAL_ui32 LOG2N,
                                ...
                                FFT_setupd *SETUPP )
{
    ...
    *Citop = xi;
    ...
    Arbot = A->realp + ( ( I * ( ( 1 << LOG2N ) - 1 ) - I ) >> 1 );
    Aibot = A->imagp + ( ( I * ( ( 1 << LOG2N ) - 1 ) - I ) >> 1 );
    Crbot = C->realp + ( ( K * ( ( 1 << LOG2N ) - 1 ) - K ) >> 1 );
    Cibot = C->imagp + ( ( K * ( ( 1 << LOG2N ) - 1 ) - K ) >> 1 );

    woff = 3 << ( ( *setupp )->log2m - LOG2N );
    ...
    for ( l = 0; l < loopcnt; l++ )
    {
        ...
        wr = ( *setupp )->real_twidp[idx];
        wi = ( *setupp )->imag_twidp[idx];
        idx += woff;
