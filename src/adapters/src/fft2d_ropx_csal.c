...
)
{
    ...
    if ( FLAG > 0 )
    {
        ...
        {
            PREFIX( _csal_fft_zb )( &split_a, ( IR << 1 ), &split_a, ( IR << 1 ),
    ...
    else
    {
        ...
        for ( i = 0; i < 2; i++ )
        {
            PREFIX( _csal_fft_zrp )( &split_a, ( IC << 1 ), &split_a, ( IC << 1 ), LOG2NR,
                                     FLAG, SETUPP );
            PREFIX( _csal_fft_zb )( &split_a, ( IC << 1 ), &split_a, ( IC << 1 ), &split_c,
        ...
        for ( i = 1; i < ( SAL_ui32 )( 1 << ( LOG2NC - 1 ) ); i++ )
        {
            PREFIX( _csal_fft_zb )( &split_a, IC, &split_a, IC, &split_c, KC, LOG2NR,
                                    FLAG, SETUPP );
            split_a.realp += ( IR << 1 );
            split_a.imagp += ( IR << 1 );
