...
)
{
    ...
    if ( ( fft_type <= SAL_FFT1D_FUNC_LB ) ||
         ( fft_type >= SAL_FFT1D_FUNC_UB ) )
    {
    ...
    if ( !( p_setup && *p_setup ) )
    {
        ...
        *p_setup = ( FFT_setup )calloc( alloc_size, sizeof( SAL_char ) );
    ...
    fft_setup_create_base( fft_type, log2m, log2n, opt_flag, p_setup, r_info );
    SAL_RINFO_CHECK_CODE_INT(r_info);

    fft_setup_init_base( p_setup, SAL_RESERVED_FLAG, r_info );
