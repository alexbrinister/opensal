...
)
{
    ...
    SAL_f64 *v0rp, *v1rp, *v2rp, *v3rp,
    *v0ip, *v1ip, *v2ip, *v3ip;

    split_a.realp = &( A->real );
    split_a.imagp = &( A->imag );

    PREFIX( _csal_fft_rad5d )( &split_a, I, &split_a, I, ( SAL_ui32 )( 1 << LOG2N ), FLAG );
    ...
    for ( i = 0; i < 5; i++ )
    {
        PREFIX( _csal_fft_zbd )( &split_a, I, &split_a, I, &split_c, ( 5*K ), LOG2N,
                                 FLAG, setupp );
