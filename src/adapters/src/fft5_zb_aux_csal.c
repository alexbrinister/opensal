...
void  PREFIX( _csal_fft5_zb )( SAL_zf32 *A, SAL_i32 I,
                               ...
                               FFT_setup *setupp )
{
    ...
    if ( LOG2N < 2 )
    {
        PREFIX( _csal_fft5_small )( A->realp, A->imagp, I,
                                    C->realp, C->imagp, K,
                                    LOG2N, FLAG );
        return;
    }
    ...
    PREFIX( _csal_fft_rad5 )( A, I, B, J, ( SAL_ui32 )( 1 << LOG2N ), FLAG );

    B_work.realp = B->realp + ( J << LOG2N );
    B_work.imagp = B->imagp + ( J << LOG2N );

    PREFIX( _csal_zvmul )( &( *SETUPP )->radix5_twids, twid_stride,
                           &B_work, J, &B_work, J,
                           ( SAL_ui32 )( 4 << LOG2N ), mul_flag );
    ...
    for ( i = 0; i < 5; i++ )
    {
        PREFIX( _csal_fft_zb )( &B_work, J, &B_work, J, &C_work, ( 5 * K ),
                                LOG2N, FLAG, setupp );
